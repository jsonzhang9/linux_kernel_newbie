...
#define PIC_M_CTRL 0x20      //主片的控制口
#define PIC_M_DATA 0x21      //主片的数据口
...
static void pic_init(void){
	...
	//打开主片上的IR0，也就是目前只接收时钟产生的中断，硬盘、键盘
...
static void make_idt_desc(struct gate_desc* p_gdesc, uint8_t attr, intr_handler function){
	...
	p_gdesc->func_offset_hign_word = ((uint32_t)function & 0xffff0000) >> 16;
}
...
static void exception_init(void){
	...
	for(i=0; i < IDT_DESC_CNT; i++){
		...
		//默认为general_init_handler,以后会用register_handler来注册具体函数
	...
	intr_name[14] = "#PE Page-Fault Exception";
	//intr_name[15] = "";
