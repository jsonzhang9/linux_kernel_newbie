...
//创建一优先级为prio的线程，线程名为name，线程执行的函数是function(func_arg)
...
static void make_main_thread(void){
	//因为main线程很早就运行了，pcb已经有了，不用在分配了
	main_thread = running_thread();
	init_thread(main_thread, "main", 31);
	//main函数是当前线程，当前线程不在thread_ready_list中，所以只能将其防区thread_all_list中
...
void schedule(){
	...
	if(cur->status == TASK_RUNNING){
		//若此线程只是cpu时间到了，就将其加入就绪队列队尾
		ASSERT(!elem_find(&thread_ready_list, &cur->general_tag));
		list_append(&thread_ready_list, &cur->general_tag);
		cur->ticks = cur->priority;
		//重新将当前线程ticks再重置为其priority
...
void thread_block(enum task_status stat){
	...
	cur_thread->status = stat;  //将其状态置为stat
	schedule();                 //将当前线程换下处理器
