...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct* child_thread, struct task_struct* parent_thread){
	//a、复制pcb所在的整个页，里面包含进程pbc信息及特权级0级的栈，里面包含了返回地址
	memcpy(child_thread, parent_thread, PG_SIZE);
	child_thread->pid = fork_pid();
	child_thread->elapsed_ticks = 0;
	child_thread->status = TASK_READY;
	child_thread->ticks = child_thread->priority;//为新进程把时间片充满
	chile_thread->parent_pid = parent_thread->pid;
	chlid_thread->general_tag.prev = child_thread->general_tag.next = NULL;
	child_thread->all_list_tag.prev = chlid_thread->all_list_tag.next = NULL;
