...
void bitmap_sync(struct partition* part, uint32_t bit_idx, uint8_t btmp_type){
	uint32_t off_sec = bit_idx / 4096;  //本i系欸但索引相对于位图的扇区偏移量
...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag){
	...
	create_dir_entry(filename, inode_no, FT_REGULAR, &new_dir_entry); //create_dir_entry只是内存操作

	//同步数据到硬盘
	//1、在目录parent_dir下安装目录项new_dir_entry, 写入硬盘后返回true,否则返回false
	...
	//创建文件需要创建相关的多个资源，若某布失败则会执行到限免的回滚步骤
	rollback:
...
int32_t file_open(uint32_t inode_no, uint8_t flag){
	...
	file_table[fd_idx].fd_pos = 0;  //每次打开文件都要将fd_pos还原为0，即让文件内指针指向开头
	file_table[fd_idx].fd_flag = flag;
	...
	}//若是读文件，或创建文件，不用理会write_deny,保持默认
	return pcb_fd_install(fd_idx);
}
...
int32_t file_write(struct file* file, const void* buf, uint32_t count){
	if((file->fd_inode->i_size + count) > (BLOCK_SIZE * 140)){
		//文件目前maximum support 512*140 = 71680字节
		printk("exceed max file_size 71860 bytes, write file failed\n");
		return -1;
	}
	...
	const uint8_t* src = buf;  //用src来指向buf中待写入的数据
	uint32_t bytes_written = 0;  //用来记录已写入的数据大小
	uint32_t size_left = count;  //用来记录未写入数据的大小
	...
	//勇敢此增量来判断是否需要分配扇区，如增量为0，表示原扇区够用
	...
	if(add_blocks == 0){
		...
		//若有增量，便涉及到新扇区分配即是否分配一级间接块表，下面要分三种情况处理
		//1、12个直接块够用
		if(file_will_use_blocks <= 12){
			...
			block_idx = file_has_used_blocks;  //第一个未使用的块，即本文件最后一个
			//已经使用的直接块的下一块
			while(block_idx < file_will_use_blocks){
				block_lba = block_bitmap_alloc(cur_part);
				if(block_lba == -1){
			...
			//第三种情况：新数据占据间接块
			ASSERT(file->fd_inode->i_sectors[12] != 0);//已经具备一级间接表
			indirect_block_table = file->fd_inode->i_sectors[12];//获取一级间接表
	...
	//用到的块地址已经收集到了all_blocks中，下面开始写数据
	...
	while(bytes_written < count){
		...
		//////////////////////////////////////////////////////////////////////
		//这是第二次遇见了，在bochs中的三元运算符是不是解码有问题？
...
//从文件file中读取count个字节到buf，返回读出的字节数，若文件尾则返回-1
