...
static void partition_format(struct partition* part){
	...
	uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR); //i结点位图占用扇区数
...
//	buf_size = ((buf_size >= sb.inode_table_sects) ? buf_size : sb.inode_table_sects) * SECTOR_SIZE;
	...
	//last_szie是位图所在最后一个扇区中，不足一扇区的部分
	
	//先将位图最后一字节到其所在扇区的结束全置为1， 即超出实际块数的部分直接置为已占用
	...
	/*由于inode_table中共有4096个inode
	 * 位图inode_bitmap正好占用一扇区
	...
	i->i_sectors[0] = sb.data_start_lba;  //由于上面的memset,i_sectors数组的其他元素都为0
...
static int search_file(const char* pathname, struct path_search_record* searched_record){
	...
	if(!strcmp(pathname, "/") || !strcmp(pathname, "/.") || !strcmp(pathname, "/..")){
		...
		searched_record->searched_path[0];  //搜索路径置空
		return 0;
	}
...
//打开或创建文件成功后，返回pcb中fd_table中的下标（文件描述符），否则返回-1
int32_t sys_open(const char* pathname, uint8_t flags){
	...
	if(pathname_depth != path_searched_depth){
		//说明没有访问到全部路径
		printk("cannot access %s: Not a diretory, subpath %s is`t exist\n", \
...
int32_t sys_close(int32_t fd){
	int32_t ret = -1;  //返回默认值为-1，即失败
	if(fd > 2){
		uint32_t _fd = fd_local2global(fd);
		ret = file_close(&file_table[_fd]);
		running_thread()->fd_table[fd] = -1; //使该描述符位可用
	}
	return ret;
}

//将buf中连续count 个字节写入文件描述符fd
//成功则返回写入的字节数，失败返回-1
...
//重置用于文件读写操作的偏移指针，成时返回新的偏移量，出错时返回-1
int32_t sys_lseek(int32_t fd, int32_t offset, uint8_t whence){
	...
	switch(whence){
		//SEEK_SET 新的读写位置是相对于当前的位置增加offset个位移量
		case SEEK_SET: 
		new_pos = offset;
		break;
		//SEEK_CUR 新的读写位置相对于当前位置增加offset个位移量
		case SEEK_CUR: //offset可正可负
		new_pos  = (int32_t)pf->fd_pos + offset;
		break;

		//SEEK_END 新的读写位置是相对于文件尺寸再增加offset个位移量
		case SEEK_END: //此情况下， offset应该是负值
...
void filesys_init(){
	...
	while(channel_no < channel_cnt){
		...
		while(part_idx < 12){
			...
			//channels数组是全局变量，默认值是0，disk属于嵌套结构
