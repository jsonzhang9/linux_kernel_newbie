...
//找到后返回true并将目录项存入dir_e,否则返回false
bool search_dir_entry(struct partition* part, struct dir* pdir, \
			const char* name, struct dir_entry* dir_e){
				...
				//写目录项的时候已经保证目录项不跨扇区
				//这样读目录项时容易处理，只真情容纳1个扇区的内存
				uint8_t* buf = (uint8_t*)sys_malloc(SECTOR_SIZE);
				struct dir_entry* p_de = (struct dir_entry*)buf;  //p_de为指向目录项
				...
				while(block_idx < block_cnt){
					...
					block_idx++;
					p_de = (struct dir_entry*)buf; //此时p_de已经指向扇区内最后一个目录
...
void dir_close(struct dir* dir){
	//根目录不能关闭
	//1、根目录自打开后就不应该关闭否则还需要再次open_root_dir();
	//2、root_dir所在的内存是低端1Mb之内，并非在堆中，free会出现问题
	if(dir == &root_dir){
		//不做任何处理直接返回
		return;
	}
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf){
	...
	while(block_idx < 140){
		...
		//若第block_idx块已经存在，将其读入内存，然后在该块中查找空目录项
